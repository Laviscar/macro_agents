...
## 4b. v1.7 叙事委员会圆桌(逼近切换时人工召开)

### 触发(自动标记,不跑 LLM)
整合阶段后自动算两路触发,写入 `storage/committee_pending/`:
- **邻近度**:`次强/最强入边权重` 跨过 `0.60/0.75/0.90` 各标记一次(滞回去抖)。
- **加速度**:挑战驱动单轮权重涨幅 ≥ `COMMITTEE_VELOCITY_DELTA`(默认 0.10)→ "⚡加速逼近",抢在邻近度变高前。

### 召开(人工门控,UI)
「叙事委员会」页三区:
- **配置席位**:加/删席位、选人格(鹰/鸽/逆向/结构/数据)、专长、LLM tier、勾选 skill(12 个库);轮次/模式;套用预设阵容;保存写回 `config/committee.yaml`。
- **待召开**:每条可「🤖 自动组阵」(规则按驱动匹配 skill,0 LLM)、显示**预估调用次数**、「🏛 召开圆桌」(点了才花 LLM)。
- **历史**:主席 memo(BLUF/方向/信心/时间窗/催化剂/证伪/仓位/分歧/证据)+ 各委员发言。

委员会结论**不改图**,只在资产上打徽章(显示在今日叙事/分歧预警,并给"将至"的资产抬高今日排序)。

### 调参(`.env`)
```
COMMITTEE_TRIGGER_LEVELS=0.60,0.75,0.90
COMMITTEE_VELOCITY_DELTA=0.10
COMMITTEE_MAX_SEATS=5
COMMITTEE_DEFAULT_ROUNDS=1
```
配置文件:`config/committee_skills.yaml`(skill 库 + 人格词表)、`config/committee.yaml`(默认席位 + 预设阵容)。
...
## 4d. v1.9 时效性(挂服务器前先懂这个)

抓取 >> 分析,所以默认会有积压。v1.9 让系统"信息新鲜、且你能看出有多新":
- **分析最新优先**(默认开,`RUN_LOOP_NEWEST_FIRST=true`):常驻 loop 先筛选/分析**最新**新闻 → 最新事件先进世界树;旧积压自然老化(不强求消化全部、不花钱分析陈年旧闻)。要按时间顺序全量消化就设 `=false`。
- **Freshness 读数**:loop 里 `freshness` stage(`FRESHNESS_LOG_SECONDS`,默认 15min)在日志打印「最新新闻 Xmin / 最新证据 / 最新驱动切换 / 最老待召开 / FRED 截至 + 待筛/待分析积压」——一眼看清时效,据此调频率。
- **待召开过期**:同一资产出现新的逼近请求 → 旧的标 `expired`(委员会页灰显、保留作记录)。
- **圆桌结论陈旧标记**:已召开 memo 被同资产更新的圆桌或更新的驱动切换取代 → 历史区标"⚠️可能已过时"(灰显保留)。

> 挂服务器建议:先 `RUN_LOOP_NEWEST_FIRST=true` + 看 freshness 日志一两天,按你能接受的"最新新闻年龄"调 `RUN_LOOP_ANALYSIS_SECONDS/BATCH`。
